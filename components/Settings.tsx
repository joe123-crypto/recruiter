...
export const Settings: React.FC<Props> = ({ company, onUpdate }) => {
  ...
  const getAppPasswordLink = (host?: string) => {
    if (!host) return 'https://www.google.com/search?q=how+to+generate+app+password';
    if (host.includes('gmail')) return 'https://myaccount.google.com/apppasswords';
    if (host.includes('outlook') || host.includes('office365')) return 'https://account.live.com/proofs/manage/additional';
    if (host.includes('yahoo')) return 'https://login.yahoo.com/account/security';
    return `https://www.google.com/search?q=how+to+generate+app+password+for+${host}`;
  };
    ...
    <div className="flex-1 flex flex-col h-full bg-slate-900/50 p-6 md:p-12 overflow-y-auto">
      <div className="max-w-2xl mx-auto w-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Scanning Parameters Panel */}
          <div className="glass-panel p-6 rounded-xl border border-slate-700">
            <h3 className="text-lg font-medium text-white mb-4 border-b border-slate-700 pb-2">Scanning Parameters</h3>

            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-1">Account Email (Sign-In)</label>
                <div className="relative mb-4">
                  <Mail className="absolute left-3 top-3 w-5 h-5 text-slate-500" />
                  <input
                    type="email"
                    className="w-full bg-slate-800/50 border border-slate-700 rounded-lg py-2.5 pl-10 pr-4 text-slate-400 cursor-not-allowed"
                    value={formData.email}
                    readOnly
                    title="This is your Google Account email used for signing in."
                  />
                </div>

                <label className="block text-sm font-medium text-slate-400 mb-1">Scanning Email (IMAP)</label>
                ...
                <div className="relative">
                  ...
                  <input
                    type="email"
                    className="w-full bg-slate-800 border border-slate-600 rounded-lg py-2.5 pl-10 pr-4 text-white focus:ring-2 focus:ring-blue-500 outline-none"
                    value={formData.imapUser || formData.email}
              ...
              <div className="pt-4 border-t border-slate-700">
                <h4 className="text-sm font-medium text-white mb-4">Email Integration (IMAP)</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-slate-400 mb-1">IMAP Host</label>
                    <input
                      type="text"
                      placeholder="imap.gmail.com"
                      className="w-full bg-slate-800 border border-slate-600 rounded-lg py-2.5 px-4 text-white focus:ring-2 focus:ring-blue-500 outline-none"
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      <label className="block text-sm font-medium text-slate-400">App Password</label>
                      <a
                        href={getAppPasswordLink(formData.imapHost)}
                        target="_blank"
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              className={`flex items-center gap-2 px-8 py-3 rounded-lg font-semibold transition-all ${saved
